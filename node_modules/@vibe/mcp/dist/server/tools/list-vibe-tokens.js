...
const SearchTokensParamsSchema = z.object({
    ...
    category: z
        .string()
        .optional()
        .describe("Filter by category: 'Spacing', 'Typography', 'Border Radius', 'Borders', 'Motion', or color themes like 'Colors (Light)'"),
    limit: z.number().optional().describe("Maximum number of results to return (default: 50)"),
    includeUsageExamples: z.boolean().optional().describe("Include CSS usage examples in the results (default: false)")
});
export const listVibeTokensTool = {
    name: "list-vibe-tokens",
    description: "Get a list of all available Vibe design tokens from the @vibe/style package. Supports optional filtering by text query, category, or limiting results. Returns token names, values, categories, and files, with optional CSS usage examples.",
    ...
    execute: async (input) => {
        ...
        try {
            ...
            if (query) {
                ...
                filteredTokens = filteredTokens.filter(token => token.name.toLowerCase().includes(searchTerm) ||
            ...
            const tokensByCategory = limitedTokens.reduce((acc, token) => {
